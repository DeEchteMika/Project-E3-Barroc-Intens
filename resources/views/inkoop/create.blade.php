...
<div class="max-w-3xl mx-auto py-8 px-4">
    ...
    <form action="{{ route('inkoop.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="productnummer" id="productnummer" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" value="{{ old('productnummer') }}" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="omschrijving" id="omschrijving" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">{{ old('omschrijving') }}</textarea>
        </div>

        <div class="grid grid-cols-2 gap-4">
            <div>
                <label for="prijs" class="block text-sm font-medium text-gray-700">Prijs</label>
                <input type="number" step="0.01" name="prijs" id="prijs" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" value="{{ old('prijs') }}">
            </div>
            <div>
                <label for="voorraad" class="block text-sm font-medium text-gray-700">Voorraad</label>
                <input type="number" name="voorraad" id="voorraad" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" value="{{ old('voorraad', 0) }}">
            </div>
        </div>

        <div class="mt-4">
            <label class="inline-flex items-center">
                <input type="checkbox" name="heeft_maler" id="heeft_maler" value="1" class="rounded text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('heeft_maler') ? 'checked' : '' }}>
